...
try {
	String fname = SettingsWn->path + "translations.csv";
...
TSearchRec sr;
int fcount = 0;
if (FindFirst(SettingsWn->path + "*.*", iAttributes, sr) == 0) {
	do {
		if ((sr.Attr & iAttributes)) {
			if (sr.Name != "." && sr.Name != "..") {
				char pathDelim = (char)System::Ioutils::TPath::DirectorySeparatorChar;
				String ifname = SettingsWn->path + sr.Name + pathDelim + "info.txt";
				if (FileExists(ifname)) {
					TStringList *info = new TStringList(this);
					info->LoadFromFile(ifname);
					for (int i = 0; i < info->Count; i++) {
						StringGrid1->Cells[fcount][i] = info->Strings[i];
					}
					fcount++;
...
*/
TStringList *info = new TStringList(this);
String ifile = SettingsWn->path + "offlinetriallist.txt";
if (FileExists(ifile)) {
	info->LoadFromFile(ifile);
	offStringGrid->RowCount = info->Count;
	for (int i = 0; i < info->Count; i++) {
		String line = info->Strings[i];
		int p = line.Pos(",");
		int col = 0;
		while (p) {
			offStringGrid->Cells[col][i] = line.SubString(0, p - 1);
			line = line.SubString(p + 1, line.Length() - p + 1);
			col++;
			p = line.Pos(",");
		}
		offStringGrid->Cells[col][i] = line;
	}
}
delete info; info = NULL;
...
if (FileExists(updateFile)) {
	...
	Label1->Text = "No shortlist available";
...
if (!LogOut->Visible) { //must login first
	if (mrOk == MessageDlg("You are not logged in. Log in now?", TMsgDlgType::mtInformation, mbOKCancel, 0)) {
		LogStatButClick(0);
	}
	else return false;
}
...
if (WizardWn->tstate == 2) { //HARVEST
	if (WizardWn->currentpage == 5) {
		if (Form3) {
			//if (Form3->Button1->Enabled) {
				Form3->Button1Click(0);
				EdChange(Form1->SpecimenId);
			//}
			delete Form3;
			Form3 = NULL;
		}
...
if (WizardWn->tstate == 2) { //HARVEST
	if (text.Pos(" Harvest")){
		WizardWn->Button3->Enabled = false; //+/- buttons
		WizardWn->Button2->Enabled = false;
	}
	else {
		WizardWn->Button3->Enabled = true;
		WizardWn->Button2->Enabled = true;
	}
...
//else z = TreeView1->InsertObject(pos, (TObject*)data);
	...
	for (int j = 1; j < sg->ColumnCount; j++) {    //iterate over the current search results headers
		...
		for (int i = 0; i < MainWn->StringGrid1->RowCount; i++) {//iterate over the main table rows
			...
			if (j == 1) {
				...
				itemR[totalcount + count]->Position->X = 0;
...
sg->RowCount = 0;
sg->ApplyStyleLookup(); //does this need to go here?

THeader *header = dynamic_cast<THeader*>(sg->FindStyleResource("header", false));
...
if (mrOk == MessageDlg("Click 'OK' remove your shortlisted items", TMsgDlgType::mtWarning, mbOKCancel, 0)) {
	TreeViewClear();
	//if (mrOk == MessageDlg("Do you want to delete your snapshot of the metadata permanently?", TMsgDlgType::mtWarning, mbOKCancel, 0)) {
...
/*String pstring = "export/samplemeasurement/csv?TrialUnitIdCSV=3,4&TraitIdCSV=1";
...
if (TreeView1->Count) {
	if (mrOk != MessageDlg("If you have not yet archived your shortlist you will lose it. Are you sure?", TMsgDlgType::mtWarning, mbOKCancel, 0)) {
...
//---------------------------------------------------------------------------
void __fastcall TForm1::sgClick(TObject *Sender) { //todo take into account the checkbox text (BUG!)
THeader *header = dynamic_cast<THeader*>(sg->FindStyleResource("header", false));
int w;
...
for (w = 0; w < header->ChildrenCount - 1; w++) {
	...
	if (headeritem == Sender) {
		...
		col = ((TStringColumn*)sg->ColumnByIndex(w));
		break;
	}
}
if (!col) {
...
else if (w == 0) {//click shortlist 'button' rather than sort it
	ShortButClick(col);
	return;
}
...
for (int row = 0; row < sg->RowCount; row++) {
	//we are maintaining the position in the list using the checkbox's tag property
	int origrow = itemR[row]->Tag;
	MySortCol->AddObject(sg->Cells[SortColumn][row], (TObject*)(itemR[origrow]));
}
...
for (int row = 0; row < sg->RowCount; row++) {
	sg->Cells[SortColumn][row] = MySortCol->Strings[row];
}
...
for (int col = 0; col < sg->ColumnCount ; col++ ) {
	if (col != SortColumn) {
		MyListCols->Clear();
		for (int row = 0; row < sg->RowCount; row++ ) {
			MyListCols->Add(sg->Cells[col][row]);
		}
		for (int rowx = 0; rowx < sg->RowCount; rowx++) {
			TCheckBox *c = dynamic_cast<TCheckBox*>(MySortCol->Objects[rowx]);
			sg->Cells[col][rowx] = MyListCols->Strings[c->Tag];

		}
	}
}
for (int row = 0; row < sg->RowCount; row++) { //todo stop the results checkbox options being recreated on every new search
	TCheckBox *c = dynamic_cast<TCheckBox*>(MySortCol->Objects[row]);
	c->Position->Y = 1 + (row) * sg->RowHeight;
	c->Tag = row;   //the tag now has the correct list position
...
ProgressBar1->Width = ClientWidth/Layout1->Scale->X - 89;
//panel2
	...
	if (combo) {
		if (combo != ItemOperation) {
			if (combo->Name == tableName) {
				...
				TStringGrid *sg = (TStringGrid*)b->TagObject;
				if (!sg) {
					if (mrOk == MessageDlg("To access this option you will need to download descriptive metadata. Do it now?", TMsgDlgType::mtConfirmation, mbOKCancel, 0)) {
						try {
...
if (hRes != S_OK) ShowMessage("KDLogue can't access the internet. Please check your connection and try again");
	...
	if (newver > cuver) {
		if (mrOk == MessageDlg("An update version " + version + " is available. Do you want to download and replace this version " + MainWn->KDVersion, TMsgDlgType::mtConfirmation, mbOKCancel, 0)) {
...
TModalResult answer = mrOk;
#ifndef _DEBUG
if (!updating) {
	if (Action != TCloseAction::caNone) {//the program is to be closed in non-debug mode
		answer = MessageDlg("Are you sure you want to exit KDLogue?", TMsgDlgType::mtConfirmation, mbOKCancel, 0);
	}
}
#endif
if (answer != mrOk) {
...
SaveDialog1->Title = "Select the csv file to be exported";
...
bool result = true;
if (Sender) {
    SaveDialog1->Title = "Select a filename for your search (.src)";
...
*/
if (BarcodeSearch.Pos("|*") && BarcodeSearch.Pos("*|")) {  //use |* to start, to end  *|
	BarcodeI = true;
	//give the timer some msec to grab the rest of the barcode (after the |*
...
if (mrOk == MessageDlg("Click 'OK' to remove " + t->Text + " from your shortlist. You will lose any modifications if you have not archived your shortlist. Continue?", TMsgDlgType::mtWarning, mbOKCancel, 0)) {
	Label1->Text = "  Item "+ t->Text.TrimLeft() + " removed from shortlist";
	RemoveTreeItem(t);
...
if (WizardWn->tstate == 0) {//PREPARE
	if (!getTreeStrFromObject(tsel, "ItemGroupId").IsEmpty()) {
		ShowMessage("You can't subsample a destination item when in PREPARE mode!");
		return;
...
for (int i = TreeView1->Count - 1; i >=0 ; i--) {
	...
	if (d->final->Values["ItemGroupId"] == searchId) {//add to the prepare seed list
...
SampleWn->ListBox2->ItemIndex = SampleWn->ListBox2->Items->Count - 1;
if (WizardWn->tstate == 0) { //PREPARE
	SampleWn->ProgressBar1->Max = SampleWn->ListBox1->Count;
	SampleWn->ProgressBar1->Value = SampleWn->CalcMeasured();
}
SampleWn->Caption = "Subsample from " + SampleWn->getselText();
SampleWn->ShowModal();
if (SampleWn->ModalResult == mrOk) {
...
if (WizardWn->currentpage == 0) {
	...
	selText = selText + "," + String(WizardWn->currentpage);
	info->Add(selText);
	info->SaveToFile(ifile);
	delete info; info = NULL;

	if (LogIn->Visible) {//logged out so disable the trial table
		WizardWn->eCheckBox->IsChecked = true;
	}
}
LoadShortButClick(0);
...
for (int i = TreeView1->Count - 1; i >= 0; i--) {
	...
	else if (ComboBox4->ItemIndex == 0) t->Visible = true;
	else { //white only
		TColorButton *but = GetButton(t);
...
if (TabControl2->TabIndex == 1 && xLabel3->Text != "Select a trial to continue") {
	if (LogIn->Visible) {//logged out
		...
		if (LogIn->Visible) {
			ShowMessage("You can't see the list of trials without first logging in");
			return;
		}
	}
	...
	onStringGrid->RowCount = MainWn->StringGrid1->RowCount;
	/*}
	else {
		Label3->Text = "Your current shortlist is selected";
		Label4->Text = "";
	...
	for (int j = 0; j < onStringGrid->ColumnCount; j++) {    //iterate over the current search results headers
		...
		for (int i = 0; i < MainWn->StringGrid1->RowCount; i++) {//iterate over the main table rows
			...
			bool found = false;
			String searchtno = MainWn->StringGrid1->Cells[GetCol("TrialId", MainWn->StringGrid1)][i];
			for (int qi = 0; qi < offStringGrid->RowCount; qi++) {
				if (offStringGrid->Cells[0][qi] == searchtno) {
					found = true;
					break;
				}
			}
			if (!found) {
				onStringGrid->Cells[j][i] = MainWn->StringGrid1->Cells[id][i];
			}
		}
	}
...
if (onStringGrid->Selected) {
	Label1->Text = "You have selected " + onStringGrid->Cells[1][onStringGrid->Selected];
	WizardWn->currentpage = 0;
	WizardBut->Visible = true;
	xLabel3->Text = "Start trial " + onStringGrid->Cells[0][onStringGrid->Selected] + " using the wizard button";
}
}
//---------------------------------------------------------------------------
void __fastcall TForm1::offStringGridSelChanged(TObject *Sender) {
if (offStringGrid->Selected) {
	Label1->Text = "You have selected " + offStringGrid->Cells[1][onStringGrid->Selected];
	WizardWn->currentpage = offStringGrid->Cells[4][offStringGrid->Selected].ToIntDef(-1);
	WizardWn->trialno = offStringGrid->Cells[0][offStringGrid->Selected].ToIntDef(-1);
	WizardBut->Visible = true;
	tLabel3->Text = "Resume trial " + String(WizardWn->trialno) + " using the wizard button";
}
}
//---------------------------------------------------------------------------
